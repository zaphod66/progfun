...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      case (blk, his) #:: xs => {
          val neighbors = newNeighborsOnly(neighborsWithHistory(blk, his), explored)
          (blk, his) #:: from(xs ++ neighbors, explored + blk)
        }
      case _ => Stream.empty
    }
  }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = 
    pathsFromStart filter (blk => done(blk._1))
